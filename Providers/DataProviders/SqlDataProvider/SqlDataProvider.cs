...
namespace Christoc.Modules.UserManager.Data
{
    ...
    public class SqlDataProvider : DataProvider
    {
        ...
        private const string ModuleQualifier = "UserManager_";
        ...
        //}
        public override IDataReader GetUsers(int moduleId)
        {
            //truyen gia tri tu view vao store pro
            return SqlHelper.ExecuteReader(ConnectionString, NamePrefix + "GetUsers", new SqlParameter("@ModuleId", moduleId));
        }
        public override void DeleteUser(int UserID)
        {
            SqlHelper.ExecuteNonQuery(ConnectionString, NamePrefix + "DeleteUser", new SqlParameter("@UserId", UserID));
        }

        public override int AddUser(User u)
        {

            return Convert.ToInt32(SqlHelper.ExecuteScalar(ConnectionString, CommandType.StoredProcedure, NamePrefix + "AddUser",
                    new SqlParameter("@UserId",u.UserId),
                    new SqlParameter("@Name", u.Name),
                    new SqlParameter("@Address", u.Address),
                    new SqlParameter("@ModuleId", u.ModuleId),
